...
function backgroundImageValue(imageFills, hash) {
  return `url('${imageFills[hash]}')`;
}
...
const createComponent = (component, imgMap, componentMap) => {
  const name = capitalizeFirstLetter(
    component.name.replace(/\[.*\]/, '').trim(),
  );

  let doc = '';
  print(`export const ${name} = (props) => {`, '');
  print(
    `  return (
  <React.Fragment>
    <style>\{\`
  ...
  const visitNode = (node, parent, lastVertical, indent) => {
    ...
    if (
      ...
      if (['FRAME', 'COMPONENT', 'INSTANCE'].indexOf(node.type) >= 0) {
        ...
        if (lastFill) {
          if (lastFill.type === 'SOLID') {
            ...
            styles.backgroundImage = backgroundImageValue(
              imgMap,
              lastFill.imageRef,
            );
      ...
      const parameterName = node.name.startsWith('[parameter]')
        ? node.name.replace('[parameter]', '').trim()
        : null;
      console.log('TEXT node parameterName', parameterName);
      ...
      content = parameterName
        ? [
            `{props.${parameterName} || <React.Fragment>${textContent.join(
              '',
            )}</React.Fragment>}`,
          ]
        : textContent;
    ...
    if (node.id !== component.id && node.name.startsWith('[component]')) {
      print(
  ...
  componentMap[component.id] = { name, doc };
};
